...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network=None, target_network=None, doubleQ=False):
        ...
        Qpredict_next_state= network.run(next_states).data
        Q_targets_next = target_network.run(next_states).data
        #Q_target = rewards + self.discount * Q_targets_next * (1 - dones)

        explorationBonus=(1 / (2*(np.sqrt((self.counts[states_np[:,0],states_np[:,1]])/100))))
        # print(explorationBonus)
        max_index = np.argmax(Qpredict_next_state, axis=1)
        Qhat = [Q_targets_next[i][max_index[i]] for i in range(max_index.shape[0])]

        val = rewards + ((1 - dones) * self.discount * Qhat) + explorationBonus
        #uodate the q_
        Q_target[np.arange(len(Q_target)),actions]=val
    ...
    def update(self, state, action, nextState, reward):
        ...
        if len(
                self.replay_memory) > self.min_transitions_before_training and self.update_amount % self.update_frequency == 0:
